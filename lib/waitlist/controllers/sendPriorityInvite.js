...
export async function sendPriorityInvite(entry, cohort, token) {
  const base = portalUrl(`/register/${cohort.slug}`)
  const registerUrl = `${base}?invite=${token}`

  try {
    await sendTemplatedEmail('waitlist_priority', entry.email, {
