...
export async function reconcilePerson(account, token, person, budget) {
  ...
  const contactId = link?.contact_id || await matchContactId(person)

  await upsertLink(account, contactId, person)
  await mergeFields(account, token, contactId, person, budget)
}
