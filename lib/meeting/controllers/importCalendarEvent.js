...
export async function importCalendarEvent(organizationId, event) {
  const { meeting, attendees, attachments } = mapCalendarEvent(event)

  if (!attendees.length) return { skipped: true }

  const id = await resolveMeeting(organizationId, meeting)

  await upsertAttachments(id, attachments)

  for (const attendee of attendees) {
    const contact = await upsertContact(organizationId, attendee)
