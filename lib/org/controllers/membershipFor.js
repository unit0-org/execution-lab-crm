...
// The user's membership. On first sign-in, link the email invite to
// their user_id (and clear the email). Returns { organizationId, role }.
export async function membershipFor(userId, email) {
  ...
  await invite.update({ user_id: userId, email: null })
