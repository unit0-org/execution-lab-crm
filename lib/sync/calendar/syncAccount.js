...
async function persistOne(supabase, ev, accountId, domains) {
  if (isCancelled(ev)) return deleteEvent(supabase, ev)
  const meetingId = await upsertEventMeeting(supabase, ev, accountId, domains)
  if (meetingId) await upsertEventAttendees(supabase, meetingId, ev.attendees)
}

async function flush(supabase, accountId, buf, totals, domains) {
  await mapWithLimit(buf, CONCURRENCY, (e) => persistOne(supabase, e, accountId, domains))
...
export async function syncCalendarAccount(supabase, account) {
  ...
  const onItem = async (ev) => {
    buf.push(ev)
    if (buf.length >= FLUSH) { await flush(supabase, account.id, buf, totals, domains); buf = [] }
  }
  ...
  if (buf.length) await flush(supabase, account.id, buf, totals, domains)
