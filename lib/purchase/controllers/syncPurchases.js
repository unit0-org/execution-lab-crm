...
// Backfill Stripe charges (with the org's key), throttled to once a day
// unless forced. Returns counts plus the latest sync time for the UI.
export async function syncPurchases(force, apiKey, organizationId) {
  const last = await lastSyncedAt(organizationId, 'purchases')

  if (!force && isFresh(last, DAY_MS)) {
    return { skipped: true, lastSyncedAt: last }
  }

  const counts = await backfillCharges(apiKey, organizationId)
  const at = await markSynced(organizationId, 'purchases')
