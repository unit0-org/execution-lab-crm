...
// A comped seat captured nothing, so no charge can be its payment. Matching
// on closeness alone claimed whatever else the registrant happened to buy
// that month — an event ticket bought two days later read as the seat's
// price and put that money in the cohort's revenue.
const capturedNothing = (registration) => registration.amount_total === 0
...
export function nearestPurchase(registration, purchases) {
  if (capturedNothing(registration)) return null
