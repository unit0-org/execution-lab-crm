...
// Checkout session supplies product names; the PaymentIntent metadata is a
// further email fallback for cal.com / Luma payments with no session.
export async function importCharge(charge) {
  const intent = await intentForCharge(charge)
