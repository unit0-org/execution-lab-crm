...
export async function mergeSignals(organizationId) {
  const [events, meetings, purchases, excluded] = await Promise.all([
    eventSignals(organizationId),
    meetingSignals(organizationId),
    purchaseSignals(organizationId),
    excludedLeadIds(organizationId)
  ])
  const byId = new Map()
