...
import { schedulePlanInstallment } from './schedulePlanInstallment'
import { dispatchRegistrationPaid }
  from '@/lib/automation/controllers/triggers/dispatchRegistrationPaid'

// Back-office for a paid registration: stamp payment, schedule the plan's
// second half when this was a deposit, re-sync the contact (now picking up
// the paid-amount facts), email, and convert the waitlist.
export async function handlePaidCheckout(session) {
  ...
  await schedulePlanInstallment(registration, cohort)
