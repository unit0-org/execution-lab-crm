...
import { buildEnrichmentMeta } from './enrichmentMeta'
...
// `actor` is the team member whose integration is writing (may be null).
export async function runEnrichment(input, t, actor) {
  const meta = buildEnrichmentMeta(input, actor)
  const people = await resolveParticipants(input.participants, t, meta)
