...
// Create a cohort resource from operator input. Links only; kind must be
// one of note|resource|recording.
export function addCohortResource(input) {
  ...
  return CohortResource.create({
    cohort_id: input.cohortId, kind: input.kind, title, url,
    session_label: clean(input.sessionLabel) || null
  }).then(() => ({ ok: true }))
}
