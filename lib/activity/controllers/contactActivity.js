...
// A contact's events, meetings, purchases, cohort registrations and synced
// emails merged into one timeline.
export async function contactActivity(contactId) {
  const [events, meetings, purchases, registrations, emails] =
    await Promise.all([
  ...
  return [...events, ...meetings, ...purchases, ...registrations, ...emails]
    .sort(byDateDesc)
}
